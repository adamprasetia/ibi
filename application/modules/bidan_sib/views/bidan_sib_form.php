...
<div class="box box-default">
	...
	<div class="box-body">
		...
		<div class="form-group form-inline">
			...
			<?php echo form_dropdown('type',$this->general_model->dropdown('bidan_'.$modulesub.'_type','Jenis Pengajuan'),set_value('type',(isset($row->type)?$row->type:'')),'id="type" class="form-control input-sm"')?>
			<small><?php echo form_error('type')?></small>
		</div>
		<hr>						
		<div class="form-group form-inline">
			<?php echo form_label('Nomor '.strtoupper($modulesub),'nomor',array('class'=>'control-label'))?>
			<?php echo form_input(array('name'=>'nomor','class'=>'form-control input-sm','maxlength'=>'20','autocomplete'=>'off','value'=>set_value('nomor',(isset($row->nomor)?$row->nomor:''))))?>
			<small><?php echo form_error('nomor')?></small>
		</div>		
		<div class="form-group form-inline">
			<?php echo form_label('Masa Berlaku '.strtoupper($modulesub),'masa_berlaku',array('class'=>'control-label'))?>
		...
		<div class="form-group">
			...
			<?php foreach ($attachment as $att): ?>
				<div class="checkbox" id="attachment_<?php echo $att->id ?>">
		...
		<div class="form-group form-inline">
			...
			<?php echo form_dropdown('status',$this->general_model->dropdown('bidan_'.$modulesub.'_status','Status'),set_value('status',(isset($row->status)?$row->status:'')),'id="status" class="form-control input-sm"')?>
