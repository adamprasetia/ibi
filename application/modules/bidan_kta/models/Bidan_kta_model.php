...
class Bidan_kta_model extends CI_Model
{
	...
	{	
		$data[] = $this->db->select(array(
			'a.*',
			'b.name as bidan_name',
			'b.tempat_lahir as tempat_lahir',
			'b.tanggal_lahir as tanggal_lahir',
			'c.name as bidan_kta_type_name',
			'd.name as bidan_kta_status_name'
		));
		$data[] = $this->db->from($this->tbl_name.' a');
		$data[] = $this->db->join('bidan b','a.bidan = b.id','left');
		$data[] = $this->db->join('bidan_kta_type c','a.type = c.id','left');
		$data[] = $this->db->join('bidan_kta_status d','a.status = d.id','left');
		$data[] = $this->search();
